...
export default function ComparisonTable({ step }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-slate-200 rounded-2xl rounded-tl-sm overflow-hidden max-w-full sm:max-w-[85%] shadow-sm">
        {/* Header */}
        <div className="px-4 py-2.5 border-b border-slate-100 flex items-center gap-2">
          <TrendingDown className="w-4 h-4 text-emerald-600" />
          <span className="text-sm font-medium text-slate-900">Price Comparison</span>
          {comparison.savings > 0 && (
            <span className="ml-auto text-xs px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700 font-medium border border-emerald-200">
                  ...
                  >
                    <td className="px-4 py-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        <span className={isBest ? 'text-emerald-700 font-medium' : 'text-slate-700'}>
                          {quote.venueName}
                        </span>
                        {quote.negotiated && (
